# src/app.py
import os
import secrets
from flask import Flask, jsonify
from flask_smorest import Api
from flask_migrate import Migrate
from flask_jwt_extended import JWTManager

from db import db
from blocklist import BLOCKLIST
import models

# Import Blueprints for different resources
from resources.item import blp as ItemBlueprint
from resources.store import blp as StoreBlueprint
from resources.tag import blp as TagBlueprint
from resources.students import blp as StudentBluePrint
from resources.course import blp as CourseBlueprint
from resources.enrollment import blp as EnrollmentBlueprint
from resources.user import blp as UserBlueprint

def create_app(db_url=None):
    app = Flask(__name__)

    # Basic Flask configuration for app behavior and OpenAPI documentation
    app.config["PROPAGATE_EXCEPTIONS"] = True
    app.config["API_TITLE"] = "Stores REST API"
    app.config["API_VERSION"] = "v1"
    app.config["OPENAPI_VERSION"] = "3.0.3"
    app.config["OPENAPI_URL_PREFIX"] = "/"
    app.config["OPENAPI_SWAGGER_UI_PATH"] = "/swagger-ui"
    app.config["OPENAPI_SWAGGER_UI_URL"] = "https://cdn.jsdelivr.net/npm/swagger-ui-dist/"
    app.config["SQLALCHEMY_DATABASE_URI"] = db_url or os.getenv("DATABASE_URL", "sqlite:///C:/Users/CS-DESKTOP-03/Downloads/section-start-code/start/data.db")
    app.config["SQLALCHEMY_TRACK_MODIFICATIONS"] = False

    # Initialize the database with Flask app
    db.init_app(app)

    # Set up Flask-Migrate to handle database migrations
    migrate = Migrate(app, db)

    # Set up Flask-Smorest API
    api = Api(app)
    
    # Configure and initialize JWT with secret key
    app.config["JWT_SECRET_KEY"] = "jose"
    jwt = JWTManager(app)
    
    # Blocklist callback to check if a token is revoked
    @jwt.token_in_blocklist_loader
    def check_if_token_in_bocklist(jwt_header, jwt_payload):
        return jwt_payload["jti"] in BLOCKLIST
    
    # Response for revoked tokens
    @jwt.revoked_token_loader
    def revoked_token_callback(jwt_header, jwt_payload):
        return(
            jsonify(
                {"description": "The token has been revoked.", "error" : "token_revoked"}
            ),
            401,
        )
        
    @jwt.needs_fresh_token_loader
    def token_not_fresh_callback(jwt_header, jwt_payload):
        return (
            jsonify(
                {
                    "description": "The token is not fresh.",
                    "error":"fresh_token_required"
                }
            )
        )
    
    # Additional claims loader to check user roles (admin or not)
    @jwt.additional_claims_loader
    def add_claims_to_jwt(identity):
        if(identity == 1):
            return {"is_admin" : True}
        return {"is_admin" : False}
    
    # Response for expired tokens
    @jwt.expired_token_loader
    def expired_token_callback(jwt_header, jwt_payload):
        return (
            jsonify({"message": "The token has expired.", "error": "token_expired"}),
            401,
        )
        
    # Response for invalid tokens (e.g., signature verification failure)
    @jwt.invalid_token_loader
    def invalid_token_callback(error):
        return (
            jsonify({"message": "Signature verification failed.", "error": "invalid_token"},
            ),
            401,
        )
        
    # Response for requests missing a JWT token
    @jwt.unauthorized_loader
    def missing_token_callback(error):
        return(
            jsonify(
                {
                    "descreption": "Request does not contain an access token.",
                    "error": "authentication_required",
                }
            ),
            401,
        )

    # Registering resource blueprints with the API
    api.register_blueprint(ItemBlueprint)
    api.register_blueprint(StoreBlueprint)
    api.register_blueprint(TagBlueprint)
    api.register_blueprint(StudentBluePrint)
    api.register_blueprint(CourseBlueprint)
    app.register_blueprint(EnrollmentBlueprint)
    api.register_blueprint(UserBlueprint)
    
    # Basic route to test if app is running
    @app.route('/')
    def home():
        return "Hello, Flask!"

    return app
